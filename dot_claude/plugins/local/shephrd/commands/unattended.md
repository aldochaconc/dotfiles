...
The command is for a master. A sheep is unattended from its first turn, since a spawned pane has
...
1. **Read the role.** `echo "$HERDR_AGENT_MASTER"`. Empty is a master, a name is a sheep. The
...
The mode lasts until the user says so, and the user says it to the master. A sheep never leaves
